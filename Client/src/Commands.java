...
public class Commands {
    ...
    public static void getTodaysOffenders(){
        String query = "SELECT * FROM ips WHERE DATE(last_attempt) = CURDATE() ORDER BY last_attempt DESC LIMIT ?;";
        ArrayList<Attempt> attempts = DatabaseFunctions.getTopResults(query, 10);
        printAttempts(attempts);
    }

    public static void getTodaysOffenders(int limit){
        String query = "SELECT * FROM ips WHERE DATE(last_attempt) = CURDATE() ORDER BY last_attempt DESC LIMIT ?;";
        ArrayList<Attempt> attempts = DatabaseFunctions.getTopResults(query, limit);
        printAttempts(attempts);
    }
